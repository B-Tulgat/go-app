...
func main() {
// Redis-client context will be refreshed every for loop inside processPrompts
...
//fmt.Printf("Status: %v , Content: %v, ID: %v \n", prompt.Status, prompt.Content, prompt.ID)

port := ports[portIdx%len(ports)]
portIdx++
sendPromptToPort(prompt, port, collection, mongoCtx)}

//fmt.Println("Nothing in Redis")
